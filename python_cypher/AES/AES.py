...
class aes_alg:
    ...
    def SubBytes(self, State):
        return [self.S_BOX[i][j] for i, j in 
            [(_ >> 4, _ & 0xF) for _ in State]]#前四位行，后四位列
    ...
    def MixColumns(self, State):
        return self.Matrix_Mul(self.MIX_C, State)  #用矩阵和中间态相乘
    # 逆列混合
    ...
    def round_key_generator(self, _16bytes_key):
        # print(_16bytes_key)
        # print('xiao',type(_16bytes_key))
        w = [_16bytes_key >> 96,    #取出高32位
            _16bytes_key >> 64 & 0xFFFFFFFF,  #取出第二高32位
            _16bytes_key >> 32 & 0xFFFFFFFF, #你懂的
            _16bytes_key & 0xFFFFFFFF] + [0]*40     #生成初始化列表，初始密钥放前四位，后四十位初始化为零
        for i in range(4, 44):      
            temp = w[i-1]
            if not i % 4:
                temp = self.SubWord(self.RotWord(temp)) ^ self.RCon[i//4-1] #s盒子对输入字的每个字节替代
            w[i] = w[i-4] ^ temp
    ...
    def _16bytes2num(cls, _16bytes):
        # 16字节转数字
        return int.from_bytes(_16bytes)

    # 数字转16字节
    def num_2_16bytes(cls, num):
        return num.to_bytes(16)

    def aes_encrypt(self, plaintext_list, key):
        plaintext_list=int(plaintext_list,16)
        key=int(key,16)
        plaintext_list = self.num_2_16bytes(plaintext_list)
        RoundKeys = self.round_key_generator(key)  #轮密钥生成
        # print('nima',type(RoundKeys))
        # print(RoundKeys)
        State = plaintext_list
        State = self.AddRoundKey(State, RoundKeys, 0)  #第一轮那个轮密钥加，在十轮迭代之前的
        ...
        State = self.SubBytes(State)
        State = self.ShiftRows(State)
        State = self.AddRoundKey(State, RoundKeys, 10)  #最后一轮不进行列混淆
        State=hex(self._16bytes2num(State))
        return State

    def aes_decrypt(self, ciphertext_list, key):
        ciphertext_list=int(ciphertext_list,16)
        key=int(key,16)
        ciphertext_list = self.num_2_16bytes(ciphertext_list)
        RoundKeys = self.round_key_generator(key)
...
if __name__ == '__main__':
    ...
    c=aes.aes_encrypt(plaintext,key)
    m=aes.aes_decrypt(c, key)
    print(f'加密后ciphertext={c}\n解密后plaintext={m}')
    # aes_alg = aes_alg()
    # key = 0x2b7e151628aed2a6abf7158809cf4f3c
    # # 加密
    # plaintext = 0x3243f6a8885a308d313198a2e0370734
    ...
    # ciphertext = aes_alg.aes_encrypt(plaintext, key)
    # print(f'ciphertext={ciphertext}')
    # # 解密
    # # int_value = int.from_bytes(ciphertext)
    # ciphertext = 0x3925841d02dc09fbdc118597196a0b32
    
    # plaintext = aes_alg.aes_decrypt(ciphertext, key)
    # print('plaintext = ' + hex(aes_alg._16bytes2num(plaintext)))
